...
• The speaker favours snapshot or golden testing methods, where tests can be written to compare outputs before and after changes.
...
• TJ's favourite project, Neo Vim, and its unique challenges (e.g. compatibility with Vim, edge cases)
...
• Falk or, a system used to retrieve data from the backend, and its potential for abuse (materializing all values)
• The Repulsive Grizzly Attack, a vulnerability discovered in Falk or that could DOS a service like Netflix
• Comparison of the name "Falk or" to the movie The Never Ending Story
...
• A team leader wraps up the meeting by stating their on track to meet OKRs and end with a positive note