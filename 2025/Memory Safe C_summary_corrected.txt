...
• Discussion of Phil C's unique feature: asynchronous, multithreaded garbage collection that frees memory later
...
• Argument in favour of using a compiler with sanitizers and intermediate representation checks (like Phil C) instead of rewriting in a different language
...
• Address sanitizer (SAN) vs. Phil C: discussion on their differences and use cases.
...
• Tag matching for neighbouring memory regions
...
• Invisibles and how it relates to memory management
...
• Using individual mallows as a workaround for arena allocators
...
• File descriptors are difficult to use with the callback pattern due to borrowing rules and lack of Zonability/copyability.
• The borrow checker prevents accessing file descriptors outside their defined region, but this can be worked around by wrapping file handles in an accessor.
...
• Miscall-related memory safety issues and performance implications
