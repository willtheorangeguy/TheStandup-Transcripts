...
• Discussion of errors in a phone, with the speaker unsure how they were labelled
• Introduction to Sentry for debugging and fixing code issues quickly
• Explanation of excess property checks and their behaviour in TypeScript
...
• A listener notes that this behaviour can cause issues with performance when dealing with thousands of components
...
• TypeScript's default behaviour as a dynamically-typed language
• Trade-offs between explicit type checking and flexibility in dynamic languages
• Design decisions in programming languages, including the choice of default behaviours
...
• Using ends in functions
• Calling enum values as strings vs. their actual value
• Auto-importing ends and potential issues
• Ends with number values (e.g. pending, decline, approved)
...
• Mention of "half C half" as an analogy for understanding the behaviour
• Acknowledgment that this is an unusual case and its behaviour may not be encountered often
...
• Rationale behind allowing accidental bug
...
• Intentional design decision preventing raw dogged behaviour
...
• Mention of ends as an alternative to maps
...
• Using object-like structures to define values instead of ends
...
• Differences between enum behaviour in other languages and TypeScript/Node.js
• User preference and DX (developer experience) factors influencing the choice between ends and object-like structures
...
• Ends automatically assign number values, which can lead to breaking code if the order of values changes
• Implicitly defined enum values are problematic and should be explicitly defined
• Quirk: using filter (Boolean) on an array with false values results in a type of "number or undefined" instead of "number[]"
• Matt Po cock's TypeScript library TS Reset
...
• TOGO (TypeScript Global Optimization) as a potential solution
• Code formatting, intentional coding, and avoiding lobbing
...
• Ends in TypeScript and potential issues
...
• Understanding JavaScript coast keyword and its limitations
...
• Discussing the behaviour of a "read-only" string
• Trying to pass a variable "A" into a function or data structure labelled as "bar"
...
• Debating whether the behaviour is a legitimate compiler bug
...
• TypeScript compiler behaviour with regard to dispatching
